...
contract RewardsContract is AccessControlUpgradeable {
    ...
    event PayoutSent(address indexed recipient, uint256 timestamp, string payoutMetadataURL, bool front, uint256 balance);
    ...
    modifier onlyAdmin() {
     ...
     function frontPayout(uint256 commitId, uint256 payoutAmount, uint256 timestamp) external onlyAdmin nonReentrant {
        require(commits[commitId].owner != address(0), "No commits exist for that id");
    ...
    function approvePayout(uint256 commitId, uint256 payoutAmount, string memory payoutMetadataURL, uint256 timestamp)  external onlyAdmin nonReentrant {
        require(commits[commitId].owner != address(0), "No commits exist for that id");
        Commit storage commit = commits[commitId];
        require(commit.spent < commit.budget, "Budget has been fully spent");
        uint256 newBalance = payoutAmount - commit.balance;
        if (newBalance > 0) {
          require(IERC20(stableToken).transferFrom(msg.sender, commit.owner, newBalance), "Transfer failed");
       ...
       emit PayoutSent(commit.owner, timestamp, payoutMetadataURL, false, commit.balance);
    }
}       
