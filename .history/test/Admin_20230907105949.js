...
describe("Admin Contract", function () {
  ...
  describe("Step 2: Check the frontPayout function", function () {
    ...
    it("should throw an error if the budget for the commit has been fully spent", async function () {
      ...
      // Make sure the budget is fully spent before calling frontPayout
      await admin.connect(owner).spendBudget(commitId, budget);
  ...
  describe("Step 3: Check the approvePayout function", function () {
    ...
    it("should throw an error if the user's debt is less than the payout amount", async function () {
      ...
      // Make sure the user's debt is less than the payout amount before calling approvePayout
      await admin.connect(owner).setUserDebt(user1.address, payoutAmount.sub(1));
    
      await expect(admin.connect(user1).approvePayout(commitId, payoutAmount, payoutMetadata, timestamp)).to.be.revertedWith("Insufficient debt");
    });
